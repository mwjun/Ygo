...
@Component({
  ...
  templateUrl: './md-signup.html',
  styleUrl: './md-signup.css'
})
export class MdSignupComponent implements OnInit {
  config;
  ...
  constructor(
    ...
    this.config = this.newsletterConfig.getConfig('md');
  }

  ngOnInit(): void {
    if (!this.cookieService.isAgeVerified()) {
      this.router.navigate(['/md-signup/age-gate']);
    }
