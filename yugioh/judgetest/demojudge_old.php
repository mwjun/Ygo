...
if(!isset($_SESSION['legal'])) header('location: /judgetest/agegate.php?l='.$language.'&test=demojudge');
// will bounce the user if not of age (
if($_SESSION['legal'] != 'yes') header('location: /judgetest/agegate.php?l='.$language.'&test=demojudge');
...
<form name="myform" action="demojudge.php" onsubmit="return validate_form(this);"  method="POST">
      ...
      <td align="right">
        <div id="lang_select_wrap" style="float: right;">
            ...
            <select name="lang_select" id="lang_select" onChange="document.location=this.value;">
                <option lang="en" value="/judgetest/demojudge.php?l=en">English</option>
                <option lang="fr" value="/judgetest/demojudge.php?l=fr">French</option>
                <option lang="de" value="/judgetest/demojudge.php?l=de">German</option>
                <option lang="it" value="/judgetest/demojudge.php?l=it">Italian</option>
            </select>
        </div>
      </td>
    </tr> -->
...
<?php
  ...
  if ($_REQUEST['submit']) {
      ...
      foreach($_POST['question'] as $key=>$value) {
          ...
          $a_result = mysql_query("SELECT * FROM questions where id = '". mysql_real_escape_string(htmlentities($value))."' and correct_answer_id = '". mysql_real_escape_string(htmlentities($_POST['answer'][$value]))."'");
          if ($a_row = mysql_fetch_array($a_result)){
            $right++;
          }
      }
      ...
      mysql_query("insert into result (email,score,qa,created,first_name,last_name,cid,version_num,test_name) values ('". mysql_real_escape_string(htmlentities($_POST['email']))."',".$per.",'".$qa."',now(),'". mysql_real_escape_string(htmlentities($_POST['fname']))."','". mysql_real_escape_string(htmlentities($_POST['lname']))."','". mysql_real_escape_string(htmlentities($_POST['cid']))."',1.0,'demojudge')");
...
An email will be sent to you within 2 weeks, with information about how you can join the Konami Demo Judge Program.
For more information about the Konami Demo Judge program, please visit: https://www.yugioh-card.com/en/judges/demo_FAQ.html";
      else
        echo "<h2>" . $per . "%</h2> You did not pass the Demo Comprehension Level 1 (DC-1) test. 
Please study the information on https://www.yugioh-card.com/en/judges/demo_FAQ.html before trying again..";
...
<?php
      ...
      $result = mysql_query("SELECT * FROM questions WHERE version_num = 1.0 AND test_name='demojudge' ORDER BY RAND() LIMIT 20");
...
?><!--
	...
	<hr>
	<font color="red">
		Passing the Demo Comprehension Level 1 (DC-1) will NOT automatically enroll you in the Konami Demo Judge Program.
	</font><br>
...
?>
    ...
    <tr>
      <td align="center" colspan="2">
        <img src="../images/YGOlegal.jpg" alt="Yu-Gi-Oh! 5D's TRADING CARD GAME" /></td>
    </tr>
    <tr>
      <td align="center" colspan="2">
