...
if (!isset($_COOKIE['legal'])) header('location: /judgetest/agegate.php?l=' . $language . '&test=rulings');
// will bounce the user if not of age (
if ($_COOKIE['legal'] != 'yes') header('location: /judgetest/agegate.php?l=' . $language . '&test=rulings');
...
<body style="font: normal .80em/2em 'trebuchet ms', arial, sans-serif;" onload="setLang('<?php echo getLang(); ?>')">
	<form name="myform" action="rulings.php" onsubmit="return validate_form(this);" method="POST">
				...
				<td align="right">
					<div id="lang_select_wrap" style="float: right;">
						...
						<select name="lang_select" id="lang_select" onChange="document.location=this.value;">
							<option lang="en" value="/judgetest/rulings.php?l=en">English</option>
							<option lang="sp" value="/judgetest/rulings.php?l=sp">Spanish</option>
							<option lang="pt" value="/judgetest/rulings.php?l=pt">Portuguese</option>
						</select>
					</div>
				</td>
			</tr>
			...
			<tr>
				<td>
					...
					if ($_REQUEST['submit']) {
						...
						$conn->query("insert into result (email,score,qa,created,first_name,last_name,cid,version_num,test_name) values ('" . $conn->real_escape_string(htmlentities($_POST['email'])) . "'," . $per . ",'" . $qa . "',now(),'" . $conn->real_escape_string(htmlentities($_POST['fname'])) . "','" . $conn->real_escape_string(htmlentities($_POST['lname'])) . "','" . $conn->real_escape_string(htmlentities($_POST['cid'])) . "',1.0,'rulings')");

						if ($per >= 80)
							echo "<h2>" . $per . "%</h2> Congratulations, you’ve passed the Rulings Comprehension Level 1 (RC-1) test!
						Your test results still need to be processed by a member of the Konami Judge Program team. 
						An email will be sent to you within 2 weeks, with information about how you can join the Konami Judge Program.
						For more information about the Konami Judge program, please visit: https://www.yugioh-card.com/en/judges/judge_FAQ.html";
						else
							echo "<h2>" . $per . "%</h2> You did not pass the Rulings Comprehension Level 1 (RC-1) test. 
       				Please study the Yu-Gi-Oh! TRADING CARD GAME Official Rulebook (https://www.yugioh-card.com/en/rulebook/index.html)  before trying again.";
					} else {
			...
			<tr>
				<td>
					...
					<?php
						...
						$result = $conn->query("SELECT * FROM questions WHERE version_num = 1.0 AND test_name='rulings' ORDER BY RAND() LIMIT 20");
	...
	-->
						<hr>
						<font color="red">
							Passing the Rulings Comprehension Level 1 (RC-1) will NOT automatically enroll you in the Konami Judge Program.
						</font><br>
